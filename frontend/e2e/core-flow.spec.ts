...
test('레포 선택 → 카드 확정 1개 경로', async ({ page }) => {
  ...
  await page.locator('.landing__actions').getByRole('button', { name: 'GitHub로 시작' }).click();
  await page.getByRole('button', { name: 'GitHub으로 이동' }).click(); // 데모: 세션 세우고 홈으로 (실서버는 Spring 콜백)
