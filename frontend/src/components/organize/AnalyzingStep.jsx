  ...
  return (
    <div className="mx-auto max-w-2xl px-8 py-9">
      <div className="mb-1 flex items-center justify-between">
        <h1 className="text-[22px] font-semibold tracking-tight text-ink-900">읽고 있습니다</h1>
        <span className="text-xs text-ink-400">누를 것이 없습니다 · 떠나도 됩니다</span>
      </div>
      ...
      <div className="mt-5 rounded-xl bg-ink-50 px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <span className="rounded-md bg-ink-900 px-2 py-1 text-xs font-medium text-white">진행 중</span>
        </div>
      </div>
      ...
      <div className="mt-4 rounded-xl border border-ink-100 p-4">
        <div className="mb-2 flex items-center justify-between text-sm">
          ...
          <span className="text-xs text-ink-400">{secondsLeft > 0 ? `약 ${secondsLeft}초 남음` : "거의 다 됐어요"}</span>
        </div>
        <div className="mb-4 h-1.5 overflow-hidden rounded-full bg-ink-100">
          <div className="h-full rounded-full bg-ink-900 transition-[width] duration-500" style={{ width: `${pct}%` }} />
        </div>
                ...
                <div className="flex items-start gap-2.5">
                  <span
                    className={`mt-0.5 grid h-4 w-4 shrink-0 place-items-center rounded border ${
                      done ? "border-ink-900 bg-ink-900" : "border-ink-300"
                    }`}
                  >
                    {done && <Check className="h-3 w-3 text-white" strokeWidth={3} />}
                  </span>
                  <div>
                    <div className={`text-sm font-medium ${done || active ? "text-ink-900" : "text-ink-400"}`}>
                      {step.title}
                    </div>
                    {done && <div className="text-xs text-ink-500">{step.doneNote(repo)}</div>}
                  </div>
                </div>
                <span
                  className={`shrink-0 rounded-md px-2 py-0.5 text-xs font-medium ${
                    done ? "bg-ink-100 text-ink-500" : active ? "bg-ink-900 text-white" : "bg-ink-100 text-ink-400"
                  }`}
                >
      ...
      <div className="mt-4 flex items-center justify-between">
        <span className="text-xs text-ink-400">창을 닫아도 계속 분석됩니다. 끝나면 알려드릴게요.</span>
        <div className="flex items-center gap-2">
          ...
          <button onClick={onCancel} className="text-sm font-medium text-ink-400 transition-colors hover:text-ink-600">
            분석 취소
          </button>
        </div>
      </div>
