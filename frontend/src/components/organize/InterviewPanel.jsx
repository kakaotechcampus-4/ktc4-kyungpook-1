  ...
  return (
    <div className="mt-3 rounded-xl border border-ink-200 bg-ink-50 p-4">
      <div className="mb-3 grid grid-cols-2 gap-3 text-xs">
        <div>
          <div className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-ink-400">찾은 것</div>
        ...
        <div>
          <div className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-ink-400">없는 것</div>
          <ul className="space-y-0.5 text-ink-600">
      ...
      <p className="mb-2 text-sm font-medium text-ink-800">{interview.question}</p>
      <textarea
        value={answer}
        onChange={(e) => setAnswer(e.target.value)}
        rows={3}
        placeholder="편하게 답변해 주세요. 다듬지 않고 그대로 저장돼요."
        className="w-full rounded-lg border border-ink-200 bg-white p-2.5 text-sm text-ink-700 placeholder:text-ink-400 focus:border-accent-400 focus:outline-none focus:ring-2 focus:ring-accent-100"
      />
      <div className="mt-2 flex items-center justify-end gap-2">
        <button onClick={onClose} className="rounded-lg px-3 py-1.5 text-xs font-medium text-ink-500 hover:bg-ink-100">
          취소
        </button>
        <button
          disabled={!answer.trim()}
          onClick={() => onSubmit(answer.trim())}
          className="rounded-lg bg-ink-900 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-ink-800 disabled:cursor-not-allowed disabled:bg-ink-200 disabled:text-ink-400"
        >
          답변 저장
        </button>
      </div>
