  ...
  return (
    <main className="main main--tight">
      ...
      {!me.data ? <Skeleton h={90} /> : (
        <div className="card row" style={{ gap: 18, padding: '20px 22px' }}>
          <span className="sidebar__avatar" style={{ width: 56, height: 56 }}>{me.data.avatarUrl && <img src={me.data.avatarUrl} alt="" />}</span>
      ...
      <div className="stat-grid">
        {[['확정한 카드', s ? `${s.confirmedCards}장` : '-'], ['정리한 레포', s ? `${s.analyzedRepos}개` : '-'], ['되묻기 턴', s ? `${s.interviewTurns}회` : '-'], ['남은 후보', s ? `${s.remainingCandidates}개` : '-']].map(([k, v]) => (
          <div key={k} className="card stat"><span className="t-12 c-2">{k}</span><b>{v}</b></div>
        ))}
      </div>
      ...
      <div className="card stack" style={{ padding: '6px 20px' }}>
