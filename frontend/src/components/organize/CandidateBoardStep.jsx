  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col px-8 py-9">
      <div className="mb-1 flex items-center justify-between">
        <h1 className="text-[22px] font-semibold tracking-tight text-ink-900">후보 {allCandidates.length}개</h1>
        <span className="text-xs text-ink-400">사람 개입 1 · 되돌릴 수 있습니다</span>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between rounded-xl bg-ink-50 px-4 py-3">
        ...
        <button className="shrink-0 rounded-lg border border-ink-200 bg-white px-3 py-2 text-xs font-medium text-ink-600 hover:bg-ink-50">
          분석 기준 보기
        </button>
      </div>
      ...
      <div className="mb-1 mt-4 flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="후보 검색"
            className="w-full rounded-lg border border-ink-200 bg-white py-2 pl-9 pr-3 text-sm text-ink-700 placeholder:text-ink-400 focus:border-accent-400 focus:outline-none focus:ring-2 focus:ring-accent-100"
          />
        </div>
        <button className="flex shrink-0 items-center gap-1 rounded-lg border border-ink-200 px-3 py-2 text-sm font-medium text-ink-600 hover:bg-ink-50">
          추천순
        ...
        <button
          ...
          className={`rounded-lg border px-3 py-2 text-sm font-medium transition-colors ${
            hideUsed ? "border-ink-900 bg-ink-900 text-white" : "border-ink-200 text-ink-600 hover:bg-ink-50"
          }`}
        >
          사용됨 숨기기
        </button>
      ...
      <div className="mb-2 text-sm font-semibold tabular-nums text-ink-900">전체 {visible.length}</div>
          ...
          return (
            <label
              ...
              className={`flex items-start gap-3 rounded-xl border p-4 transition-colors ${
                disabled
                  ...
                  : checked
                  ? "cursor-pointer border-ink-300 bg-ink-50"
            ...
            >
              <input
                ...
                className="mt-0.5 h-4 w-4 rounded border-ink-300 text-accent-600 focus:ring-accent-200"
              />
              <div className="min-w-0 flex-1">
                  ...
                  <div className="flex min-w-0 items-center gap-2">
                    <span
                      className={`shrink-0 rounded-md px-1.5 py-0.5 text-xs font-medium ${
                        c.type === CANDIDATE_TYPE.COMMIT_CLUSTER ? "bg-ink-100 text-ink-600" : "bg-ink-900 text-white"
                      }`}
                    >
                      {CANDIDATE_TYPE_LABEL[c.type](c.ref)}
                    </span>
                    <span className={`truncate text-sm font-semibold ${disabled ? "text-ink-400" : "text-ink-900"}`}>
                      {c.title}
                    </span>
                    {c.lowCardWorth && (
                ...
                <p className="mt-1 text-xs text-ink-500">추천 이유 · {c.reason}</p>
      ...
      <div className="sticky bottom-0 mt-3 flex items-center justify-between rounded-xl border border-ink-200 bg-white px-4 py-3 shadow-popover">
        <div className="text-sm">
          <div className="font-semibold tabular-nums text-ink-900">
            후보 {selectedCount}개 선택
            {excludedCount > 0 && ` · ${excludedCount}개 제외`}
          </div>
          <div className="text-xs text-ink-400">확정한 후보의 코드만 깊게 읽습니다 · 예상 2분</div>
        </div>
